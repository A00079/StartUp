...
module.exports = {
    ...
    editDetails: (data, userID, callBack) => {
        var sql = 'UPDATE shopaddress,shop  SET shop.phone = ?, shopaddress.shopNo = ?, shopaddress.complex = ?, shopaddress.landmark = ?, shopaddress.street = ?, shopaddress.area = ?, shopaddress.city = ? WHERE shop.id = shopaddress.shop_id AND shop.id = ?'

        var insertSql = [data.phone, data.shopNo, data.complex, data.landmark, data.street, data.area, data.city, userID]
